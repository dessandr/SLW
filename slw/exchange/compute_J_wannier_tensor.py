"""Compute spinor exchange tensor J^{ab}(R) from Wannier90 up/down hr.dat.
...
from slw.exchange.build_spinor_soc_hr import _basis_permutation_from_win
from slw.exchange.compute_J_epr_kspace import _compute_j_direct, _find_nearest_neighbours_from_epr
from slw.exchange.compute_J_epr_tensor import (
    _apply_model_soc,
...
from slw.exchange.diagnose_J_epr_kspace import _build_hk_from_epr, _full_k_mesh, _load_slices
from slw.exchange.eph_epr_wrapper import _unit_scale_to_ev
from slw.exchange.lkag_solver import get_cfr_ozaki_mesh, get_cfr_pole_mesh, get_semicircle_contour
from slw.exchange.plot_epr_soc_bands import _win_projection_groups
...
def run(args):
    ...
    print(
        ...
        f"nE={len(energy_mesh)} axes={axes} nproc={args.nproc} structure={structure_source}",
        flush=True,
    )
    t0 = time.time()
    extra = None
    if args.kernel == "scalar":
        _ensure_scalar_kernel_is_soc_free(args)
        j_mev, scalar_trace_acc, _kdata, exe_info = _compute_j_direct(
            hk_up,
            hk_dn,
            slices,
            pair_meta,
        ...
        tensor = _scalar_j_to_tensor(j_mev, axes)
        trace_acc = _scalar_trace_to_tensor(scalar_trace_acc, axes)
    elif args.kernel == "direct":
        tensor, trace_acc, exe_info = _compute_tensor_direct(
            h_spin, slices, pair_meta, kpts, energy_mesh, args.efermi, axes, nproc=args.nproc, collinear_override=args.collinear_override,
        ...
        tensor, trace_acc, extra, exe_info = _compute_tensor_tb2j(
            h_spin, slices, pair_meta, kpts, energy_mesh, args.efermi, axes, collinear_override=args.collinear_override,
            dynamic_soc=dynamic_soc_val,
            hk_up=hk_up,
            hk_dn=hk_dn,
            d_idx=d_idx,
