"""Refactored Analytic dJ/du Tensor Calculator for EPR (PROPER STRUCTURE).
...
from slw.exchange.eph_epr_wrapper import _read_meta, _unit_scale_to_ev
from slw.exchange.compute_J_epr_kspace import (
    _mirror_indices,
    _orbit_label_map,
    _split_chunks,
)
from slw.exchange.compute_J_epr_tensor import _apply_model_soc
from slw.exchange.compute_dJ_epr_kspace import (
    _atom_labels,
...
from slw.exchange.compute_J_wannier_tensor import (
    _infer_collinear_slices_from_win,
    _load_spinor_hr_hk,
    _slice_file_order_summary,
    _spinor_slice_summary,
)
from slw.exchange.lkag_solver import (
    _accumulate_dJ_tensor_from_GR_njit,
    get_cfr_ozaki_mesh,
    get_cfr_pole_mesh,
    get_semicircle_contour,
    lkag_tensor_deriv_trace_njit,
)
...
def _load_slices_for_global_atoms(args, dim, atom_ids):
    ...
    slice_keys = {int(k) for k in slices.keys()}
    ...
    if use_local_order:
        ...
        if missing_local:
            raise ValueError(
                ...
                f"{missing_local}; available keys={sorted(int(k) for k in slices.keys())}"
            )
    ...
    if missing_atoms:
        raise ValueError(
            ...
            f"{missing_atoms}; available keys={sorted(int(k) for k in slices.keys())}. "
...
def run_analytic(args):
    ...
    if spinor_input:
        ...
        print(f"[dJ-epr-tensor] Loading base H(k) up/dn from EPR", flush=True)
        ...
        print(f"[dJ-epr-tensor] Building spinor H(k) from collinear up/dn...", flush=True)
...
def _save_results(args, results, pair_meta, labels, tensor_axes, disp_axes, qmesh, *, complete=True):
    ...
    from slw.exchange.compute_dJ_epr_kspace import _rp_grid
    ...
    with h5py.File(out_h5, "w") as h5:
        ...
        h5.attrs["n_completed_target_axis"] = int(len(results))
        ...
        target_ids = sorted(list(set(k[0] for k in results.keys())))
