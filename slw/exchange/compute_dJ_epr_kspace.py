"""Compute dJ/du directly in EPR k,q space for Rp=(0,0,0).
...
except Exception:  # pragma: no cover - optional runtime accelerator control
...
from slw.exchange.diagnose_J_epr_kspace import _find_nearest_neighbours_from_epr, _full_k_mesh, _load_slices
...
def run(args):
    ...
    print(f"[dJ-epr-kspace] assemble dJ: bonds={len(pair_meta)} targets={len(target_ids)} axes={axes} nE={len(energy_mesh)}")
    djr, exe_info = _compute_dj(pre, slices, pair_meta, target_ids, axes, energy_mesh, args.nproc, args.omp_threads)
