...
"""Temporary typed bridge to the quarantined numerical reference kernels.

This module deliberately does not call an argparse parser, mutate ``sys.argv``,
or invoke a legacy ``main``.  It is the single migration boundary that can be
deleted once the four numerical kernels return native result objects.
"""
...
class ReferenceArtifacts:
    """Paths expected from one completed reference-kernel run."""
...
def _core_options(request: ExchangeRequest) -> dict[str, Any]:
    ...
    options = {
        **files,
        ...
        # Absolute product paths prevent retained writers from joining a
...
def build_namespace(request: ExchangeRequest) -> tuple[str, str, argparse.Namespace]:
    """Return ``(module, callable, namespace)`` for one reference kernel."""

    if request.calculation is ExchangeCalculation.J:
        if request.source is ExchangeSource.EPR and not request.ltensor:
            module = "slw.exchange.legacy.reference.compute_J_epr_kspace"
            function = "run"
            defaults = _J_SCALAR_EPR_DEFAULTS
        elif request.source is ExchangeSource.EPR:
            module = "slw.exchange.legacy.reference.compute_J_epr_tensor"
            function = "run"
            defaults = _J_TENSOR_EPR_DEFAULTS
        else:
            module = "slw.exchange.legacy.reference.compute_J_wannier_tensor"
            function = "run"
            defaults = _J_WANNIER_DEFAULTS
    elif request.ltensor:
        module = "slw.exchange.legacy.reference.compute_dJ_epr_tensor"
        function = "run_analytic"
        defaults = _DJ_TENSOR_DEFAULTS
    else:
        module = "slw.exchange.legacy.reference.compute_dJ_epr_kspace"
        function = "run"
        defaults = _DJ_SCALAR_DEFAULTS

    options = dict(defaults)
    advanced = request.legacy_options.as_dict()
...
def execute(
    ...
    *,
    ...
    """Execute one quarantined kernel without entering its CLI driver."""
    ...
    if mpi:
        if not (
            ...
            raise ValueError("only tensor dJ has an MPI reference kernel")
        module_name = "slw.exchange.legacy.reference.compute_dJ_epr_tensor_mpi"
        function_name = "run_mpi"
    module = importlib.import_module(module_name)
    function = getattr(module, function_name)
    result = function(namespace, comm=comm) if mpi else function(namespace)
    if isinstance(result, int) and result != 0:
        raise RuntimeError(f"reference kernel returned exit code {result}")
    ...
    if request.legacy_options.as_dict().get("no_h5", False):
        paths = [path for path in paths if Path(path) != Path(request.output.h5_path)]
    return ReferenceArtifacts(paths=tuple(paths))


__all__ = ["ReferenceArtifacts", "build_namespace", "execute"]
