...
"""Plot model-SOC spinor bands built directly from EPR up/down H(k)."""
...
from slw.exchange.legacy.diagnose_J_epr_kspace import _build_hk_from_epr
...
def _resolve_soc_groups_from_win(args, nwan):
    ...
    if legacy_groups and abs(float(args.lambda_te)) > 0.0:
        specs.append((_clean_species(args.soc_element), "p", float(args.lambda_te)))
    elif abs(float(args.lambda_te)) > 0.0:
        specs.append((_clean_species(args.soc_element), "p", float(args.lambda_te)))
    if not specs:
        return [], None
