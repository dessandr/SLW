"""Compute bond-resolved LKAG J directly from EPR electron_wannier H(k)."""
...
def _debug_spglib_orbit_maps(neighbours, pos, rotations, translations, key_to_idx, apply_op, *, labels=None, shell=None):
    ...
    for iop, (rot, trans) in enumerate(zip(rotations, translations)):
        print(
            f"  op={iop:03d} det={int(round(np.linalg.det(rot))):+d} "
...
def _debug_epr_position_payload(epr_path, lattice_ang, tau_frac):
    with h5py.File(epr_path, "r") as h5:
        keys = sorted(str(k) for k in h5["basic_data"].keys()) if "basic_data" in h5 else []
...
def run(args):
    ...
    print(f"[J-epr-kspace] computing J for {len(pair_meta)} directed bonds, nE={len(energy_mesh)}, nproc={int(args.nproc)}")
    j_mev, trace_acc, kdata, exe_info = _compute_j_direct(
        hk_up,
        hk_dn,
        slices,
        pair_meta,
