"""qe2pert EPR Hamiltonian and electron-phonon provider utilities."""
import os
import pickle
...
from slw.epc.epr_io import energy_scale_to_ev as _unit_scale_to_ev
from slw.epc.epr_io import read_epr_metadata as _read_meta
...
class EPRNativeProvider:
    ...
    def close(self):
        for fh in self._h5.values():
            try:
                fh.close()
            except Exception as exc:  # noqa: BLE001 - close every cached handle
                print(f"[eph-provider][warn] failed to close cached HDF5 file: {exc}")
        self._h5 = {}
